...
fn get_mode(ath: Rc<String>, orig: Unit, msg: Unit, kern: &Mutex<Kern>) -> ThreadAsync<Result<Option<Mode>, KernErr>> {
    thread!({
        if let Some((res, ath)) = read_async!(msg, ath, orig, kern)? {
            if let Some(s) = res.as_str() {
                if s.as_str() == "get.mode" {
                    return Ok(Some(kern.lock().term.mode.clone()))
                }
            }
        }
        Ok(None)
...
pub fn term_hlr(msg: Msg, _serv: ServInfo, kern: &Mutex<Kern>) -> ServHlrAsync {
    thread!({
        ...
        // get mode
        if let Some(mode) = thread_await!(get_mode(ath.clone(), msg.msg.clone(), msg.msg.clone(), kern))? {
            let msg = Unit::map(&[
