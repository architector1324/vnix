...
pub fn vnix_entry(mut kern: Kern) -> Result<(), KernErr> {
    ...
    'login: loop {
        let path = Unit::parse("@task.zen.min.login".chars()).map_err(|e| KernErr::ParseErr(e))?.0;
    ...
    // zen
    let path = Unit::parse("@task.zen.min.desk.load".chars()).map_err(|e| KernErr::ParseErr(e))?.0;
